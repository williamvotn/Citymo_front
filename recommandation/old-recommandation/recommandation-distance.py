...
# Créer une nouvelle colonne 'distance' correspondant à la distance en mètres
# entre les biens immobiliers par rapport au bien de référence
df['distance'] = df.apply(lambda row: geodesic((row['latitude'], row['longitude']),
                                              (df.iloc[reference_index]['latitude'], df.iloc[reference_index]['longitude'])).meters, axis=1)

# Sélectionner les colonnes pour le calcul de la similarité (en utilisant la colonne 'distance' à la place de 'latitude' et 'longitude')
columns_to_compare = ['valeur_fonciere', 'surface_reelle_bati', 'nombre_pieces_principales', 'distance']
...
plt.savefig('recommandation/old-recommandation/top10kdistance.png')

# Supprimer la colonne 'distance' avant de sauvegarder le CSV
df_sorted.drop(columns=['distance'], inplace=True)
...
df_sorted.to_csv('recommandation/old-recommandation/top10kdistance.csv', index=False)
